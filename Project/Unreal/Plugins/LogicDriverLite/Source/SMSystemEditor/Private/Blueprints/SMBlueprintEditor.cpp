...
FGraphAppearanceInfo FSMBlueprintEditor::GetGraphAppearance(UEdGraph* InGraph) const
{
	...
	if (GetStateMachineBlueprint())
	{
		...
		{
			if (GraphClass->IsChildOf<USMIntermediateGraph>())
			{
...
void FSMBlueprintEditor::ExtendMenu()
{
	...
	ISMSystemEditorModule& SMBlueprintEditorModule = FModuleManager::LoadModuleChecked<ISMSystemEditorModule>(LOGICDRIVER_EDITOR_MODULE_NAME);
